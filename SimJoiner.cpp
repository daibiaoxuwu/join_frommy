...
int SimJoiner::searchJaccard(char *query_split, int id2, Trie jacTrie, double threshold, vector<JaccardJoinResult> &result)
{
	...
	for(int i = 0; i < line_count; ++i){
		...
		if(jaccard>=threshold)
            result.emplace_back(create_JaccardJoinResult(i, id2, jaccard));
...
int SimJoiner::createEDIndex(const char *filename, int q, Trie edTrie){
	...
	for(line_count=0;fgets(buf,1024,file);++line_count){
		...
		char_count[line_count] = (int)strlen(buf);
		if(lines[line_count] == nullptr) lines[line_count] = new char[258];
		memcpy(lines[line_count],buf,strlen(buf)+1);
	...
	if(query_count - q + 1 <= threshold * q){
		for(int i = 0; i < line_count; ++ i){
			...
			if (ed_value <= threshold)
                result.emplace_back(create_EDJoinResult(i, id2, ed_value));
...
int SimJoiner::joinED(const char *filename1, const char *filename2, unsigned threshold, vector<EDJoinResult> &result) {
	...
	for(line_count=0;fgets(buf,1024,file);++line_count){
        ...
        searchED(buf, line_count, edTrie, threshold, result);
    }
    //CP
    sort(result.begin(), result.end());//TODO: need unique?
    result.resize(unique(result.begin(), result.end()) - result.begin());
